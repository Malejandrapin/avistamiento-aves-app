...
import { Aguilas } from '../../interfaces/clasesAves/aguilas';

@Component({
  imports: [CommonModule, ReactiveFormsModule],
  selector: 'app-aguila-form.component',
  styleUrl: './aguila-form.component.scss',
  templateUrl: './aguila-form.component.html',
})
export class AguilaFormComponent {
   registered = output<Aguilas>();
    readonly conservationStatus = Object.values(Conservationstatus);
    readonly habitatType = Object.values(HabitatType);
    readonly behaviors = Object.getOwnPropertyNames(Aguilas.prototype)
    ...
    constructor(
      ...
      this.aguilaForm = this.formBuilder.group({
        commonName: ['Aguila', Validators.required],
        scientificName: [''],
    ...
    submit(): void {
      this.errors = [];
      if (this.aguilaForm.invalid) {
        this.aguilaForm.markAllAsTouched();
        this.errors = ['Completa los campos obligatorios correctamente.'];
        return;
      }
  
      const raw = this.aguilaForm.getRawValue();
