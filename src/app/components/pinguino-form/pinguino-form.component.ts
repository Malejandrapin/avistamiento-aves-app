...
import { Pinguinos } from '../../interfaces/clasesAves/pinguinos';
...
@Component({
  selector: 'app-pinguino-form',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './pinguino-form.component.html',
  styleUrl: './pinguino-form.component.scss',
})
export class PinguinoFormComponent {
  registered = output<Pinguinos>();
  readonly conservationStatus = Object.values(Conservationstatus);
  readonly habitatType = Object.values(HabitatType);
  readonly behaviors = Object.getOwnPropertyNames(Pinguinos.prototype)
    .filter((method) => method !== 'constructor');
  readonly pinguinoForm;
  errors: string[] = [];
  ...
  constructor(
    ...
    this.pinguinoForm = this.formBuilder.group({
      commonName: ['Pingüino', Validators.required],
      scientificName: [''],
  ...
  submit(): void {
    this.errors = [];
    if (this.pinguinoForm.invalid) {
      this.pinguinoForm.markAllAsTouched();
      this.errors = ['Completa los campos obligatorios correctamente.'];
      return;
    }

    const raw = this.pinguinoForm.getRawValue();
  ...
  private resetForm(): void {
    this.pinguinoForm.reset({
      commonName: 'Pingüino', scientificName: '', conservationStatus: '',
