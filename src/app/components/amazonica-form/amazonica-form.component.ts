...
import { Amazonicas } from '../../interfaces/clasesAves/amazonicas';

@Component({
  imports: [CommonModule, ReactiveFormsModule],
  selector: 'app-amazonica-form.component',
  styleUrl: './amazonica-form.component.scss',
  templateUrl: './amazonica-form.component.html',
})
export class AmazonicaFormComponent {
  registered = output<Amazonicas>();
      readonly conservationStatus = Object.values(Conservationstatus);
      readonly habitatType = Object.values(HabitatType);
      readonly behaviors = Object.getOwnPropertyNames(Amazonicas.prototype)
        .filter((method) => method !== 'constructor');
      readonly amazonicaForm;
      errors: string[] = [];
      ...
      constructor(
        ...
        this.amazonicaForm = this.formBuilder.group({
          commonName: ['Amazonica', Validators.required],
          scientificName: [''],
      ...
      submit(): void {
        this.errors = [];
        if (this.amazonicaForm.invalid) {
          this.amazonicaForm.markAllAsTouched();
          this.errors = ['Completa los campos obligatorios correctamente.'];
          return;
        }
    
        const raw = this.amazonicaForm.getRawValue();
